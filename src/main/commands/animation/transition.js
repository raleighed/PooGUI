...
module.exports = {
    ...
    execute: async function (args) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (!f.audio) audioInputs.push({ label: `${audioInputs.length + files.length}:a`, filters: `-f lavfi -i "anullsrc=cl=stereo:r=44100"` });
        }
        ...
        const inArgs = [
            ...inputs.map(f => `${f.filters ? `${f.filters} ` : ""}-i "${f.path}"`),
            ...audioInputs.filter(f => f.filters).map(f => f.filters)
        ].join(" ");
